...
# Executable
TARGET = kotlin
DEP 	 = $(PARSER_OBJ) $(LEXER_OBJ) -I$(INTER_INC) -I$(PARSER_INC) -I$(AST_INC) -I$(ARENA_INC) -I$(ERROR_INC) -I$(HASH_INC) -lintercode -lsymboltable -L$(INTER_LIB) -last -lhashtable -L$(AST_LIB) -larena -L$(ARENA_LIB) -lerror -L$(ERROR_LIB) -lhashmap -L$(HASH_LIB)
...
$(TARGET): $(SRC) | error arena hashmap hashtable ast parser lexer symboltable intercode
	@$(CC) -o $(BINDIR)/$(TARGET) $< $(DEP)
	@echo "Built the target binary $(BINDIR)/$(TARGET)"

# Rule to compile source files
$(BUILDDIR)/$(TARGET).o: $(SRC) | error arena hashmap hashtable ast parser lexer symboltable intercode
	@$(CC) $(DEBUG) -o $@ $< $(DEP)
	@echo "Compiled $@"
...
# Clean rule
clean:
	@rm -f $(BUILDDIR)/*.o $(ARENA_DIR)/build/* $(AST_DIR)/build/* $(ERROR_DIR)/build/* $(INTER_DIR)/build/* $(HASH_DIR)/build/* $(LEXER_DIR)/build/*.c $(PARSER_DIR)/build/* $(TARGET)
...
.PHONY: all install root-install build clean clean_test arena ast error intercode lexer parser symboltable tarena run-tarena
