...
void emit_syscall(int syscall_num, FILE *output) {
  fprintf(output, "li $v0, %d\n", syscall_num);  // Load syscall number into $v0
...
void translate_op(Address arg, char* reg, FILE* output) {
  switch (arg->type) {
    case AT_IntConst:
      fprintf(output, "li %s, %ld", reg, arg->value.val_int);
      break;
    case AT_UIntConst:
      fprintf(output, "li %s, %lu", reg, arg->value.val_uint);
      break;
    case AT_Char:
      fprintf(output, "li %s, %d", reg, arg->value.val_char);
      break;
    case AT_String:
      fprintf(output, "la %s, %s", reg, arg->value.label);
      break;
    case AT_Temp:
    ...
    case AT_Label:
      ...
      break;
    default:
      fprintf(output, "Invalid Address Type");
      break;
  }
}
...
void emit_instruction(const char* instruction, Address arg1, Address arg2, Address arg3, FILE* output) {
  ...
  if (arg1->type != AT_Empty) {
    translate_op(arg1, reg1, output);
  }
  if (arg2->type != AT_Empty) {
    translate_op(arg2, reg2, output);
  }
  if (arg3->type != AT_Empty) {
